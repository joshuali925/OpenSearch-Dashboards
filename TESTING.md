...
* Flaky tests need to be fixed to reduce maintenance efforts. Here are some ways to avoid flaky tests:
    ...
    * Avoid hard coded delays in tests, use `cy.get()` to wait for UI or `cy.wait()` to wait for network requests.
