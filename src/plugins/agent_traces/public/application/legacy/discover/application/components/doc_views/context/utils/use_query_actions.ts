    ...
    try {
      ...
      toastNotifications.addDanger({
        title: i18n.translate(
          'agentTraces.discover.context.unableToLoadAnchorDocumentDescription',
          {
  ...
  const fetchSurroundingRows = useCallback(
    async (type: SurrDocType, count: number, filters: Filter[], anchor?: OpenSearchHitRecord) => {
      try {
        ...
        toastNotifications.addDanger({
          title: i18n.translate(
            'agentTraces.discover.context.unableToLoadSurroundingDocumentDescription',
            {
  ...
  const fetchAllRows = useCallback(
    async (predecessorCount: number, successorCount: number, filters: Filter[]) => {
      try {
        ...
        toastNotifications.addDanger({
          title: i18n.translate('agentTraces.discover.context.unableToLoadDocumentDescription', {
