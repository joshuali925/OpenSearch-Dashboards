...
describe('DatasetAccordionList', () => {
  ...
  const mockOnViewInAgentTraces = jest.fn();
  ...
  const defaultProps: DatasetAccordionListProps = {
    ...
    onViewInAgentTraces: mockOnViewInAgentTraces,
  ...
  describe('Basic Rendering', () => {
    ...
    it('renders view in agent traces buttons only for datasets with logs', () => {
      render(<DatasetAccordionList {...defaultProps} />);

      expect(
        screen.getByTestId('dataset-view-in-agentTraces-button-dataset-1')
      ).toBeInTheDocument();
      expect(
        screen.getByTestId('dataset-view-in-agentTraces-button-dataset-2')
      ).toBeInTheDocument();
      expect(
        screen.queryByTestId('dataset-view-in-agentTraces-button-dataset-3')
      ).not.toBeInTheDocument();
    });

    it('renders view in agent traces button text only for datasets with logs', () => {
      render(<DatasetAccordionList {...defaultProps} />);
  ...
  describe('View in Agent Traces Functionality', () => {
    it('calls onViewInAgentTraces with correct parameters when button is clicked', () => {
      render(<DatasetAccordionList {...defaultProps} />);

      const viewButton = screen.getByTestId('dataset-view-in-agentTraces-button-dataset-1');
      fireEvent.click(viewButton);

      expect(mockOnViewInAgentTraces).toHaveBeenCalledWith(mockDatasets[0], mockLogs);
    });

    it('calls onViewInAgentTraces multiple times for different datasets', () => {
      render(<DatasetAccordionList {...defaultProps} />);

      fireEvent.click(screen.getByTestId('dataset-view-in-agentTraces-button-dataset-1'));
      fireEvent.click(screen.getByTestId('dataset-view-in-agentTraces-button-dataset-2'));

      expect(mockOnViewInAgentTraces).toHaveBeenCalledTimes(2);
      expect(mockOnViewInAgentTraces).toHaveBeenNthCalledWith(1, mockDatasets[0], mockLogs);
      expect(mockOnViewInAgentTraces).toHaveBeenNthCalledWith(2, mockDatasets[1], [mockLogs[0]]);
    });
  });
