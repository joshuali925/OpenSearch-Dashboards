...
jest.mock('./url_builder', () => ({
  buildAgentTracesLogsUrl: jest.fn(() => 'https://example.com/logs?span=span-1'),
...
describe('SpanLogsTab', () => {
  ...
  describe('View in Discover Logs button', () => {
    it('renders the button', () => {
      ...
      expect(screen.getByText('View in Discover Logs')).toBeInTheDocument();
      expect(
        screen.getByTestId('span-logs-view-in-agentTraces-button-logs-dataset-id')
      ).toBeInTheDocument();
    });
    ...
    it('navigates to logs URL when datasets are available', () => {
      ...
      fireEvent.click(screen.getByTestId('span-logs-view-in-agentTraces-button-logs-dataset-id'));
      ...
      expect(urlBuilder.buildAgentTracesLogsUrl).toHaveBeenCalledWith(
