...
import { buildAgentTracesLogsUrl, getTimeRangeFromTraceData } from './url_builder';
...
export const TraceLogsTab: React.FC<TraceLogsTabProps> = ({
  ...
  const handleViewInAgentTraces = (logDataset: Dataset, logs: LogHit[]) => {
    try {
      const timeRange = getTimeRangeFromTraceData(logs);

      const url = buildAgentTracesLogsUrl({
        traceId,
        logDataset,
        timeRange,
      });
  ...
  if (logDatasets.length === 0) {
    return (
      <EuiPanel>
        <EuiTitle size="s">
          <h3>
            {i18n.translate('agentTraces.traceLogsTab.title', {
              defaultMessage: 'Logs',
            })}
          </h3>
        </EuiTitle>
  ...
  return (
    <div>
      <EuiPanel>
        <EuiTitle size="s">
          <h3>
            {i18n.translate('agentTraces.traceRelatedLogsTab.title', {
              defaultMessage: 'Related logs',
            })}
          </h3>
        </EuiTitle>
        <EuiText size="s" color="subdued">
          {i18n.translate('agentTraces.traceLogsTab.description', {
        ...
        <DatasetAccordionList
          ...
          onViewInAgentTraces={handleViewInAgentTraces}
