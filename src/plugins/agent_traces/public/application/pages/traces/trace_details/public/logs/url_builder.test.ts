...
describe('url_builder', () => {
  ...
  describe('buildAgentTracesLogsUrl', () => {
    ...
    it('should build URL with trace ID only', () => {
      ...
      const result = buildAgentTracesLogsUrl(params);

      expect(result).toContain('https://example.com/app/agentTraces/logs/#/');
    ...
    it('should handle custom base path', () => {
      mockLocation.pathname = '/custom-base/app/agentTraces';
      ...
      expect(result).toContain('https://example.com/custom-base/app/agentTraces/logs/#/');
    });
