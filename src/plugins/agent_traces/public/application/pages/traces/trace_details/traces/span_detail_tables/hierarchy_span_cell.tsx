...
export const HierarchySpanCell = ({
  ...
  useEffect(() => {
    if (isRowSelected) {
      setCellProps?.({
        className: ['treeCell--firstColumn', 'agentTracesSpanDetailTable__selectedRow'],
      });
    } else {
  ...
  const ExpandCollapseIcon = () =>
    item?.children && item.children.length > 0 ? (
      <EuiIcon
        ...
        className="agentTracesSpanDetailTable__expandIcon"
        data-test-subj="treeViewExpandArrow"
      />
    ) : (
      <EuiIcon type="empty" className="agentTracesSpanDetailTable__hiddenIcon" />
    );
  ...
  const cellContent = (
    <div className="agentTracesSpanDetailTable__hierarchyCell" style={{ paddingLeft: indentation }}>
      <ExpandCollapseIcon />
      <SpanContent />
    </div>
  );
  ...
  return disableInteractions || !item ? (
    ...
    <button
      ...
      className="agentTracesSpanDetailTable__flyoutButton"
