...
export const SpanCell = ({
  ...
  useEffect(() => {
    if (
      ...
      setCellProps?.({ className: 'agentTracesSpanDetailTable__selectedRow' });
    } else {
      setCellProps?.({});
    }
    ...
    case 'status.code':
      return value === 2 ? (
        <EuiText color="danger" size="s">
          {i18n.translate('agentTraces.spanDetailTable.errors.yes', {
            defaultMessage: 'Yes',
          })}
        </EuiText>
      ) : (
        i18n.translate('agentTraces.spanDetailTable.errors.no', {
          defaultMessage: 'No',
        })
      );
    case 'spanId':
