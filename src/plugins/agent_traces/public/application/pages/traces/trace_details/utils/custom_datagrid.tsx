...
const FullScreenWrapper: React.FC<FullScreenWrapperProps> = ({
  ...
  return (
    <EuiOverlayMask>
      <div className="agentTracesCustomDataGrid__fullScreenWrapper">
        <EuiButtonIcon
          iconType="cross"
          aria-label={i18n.translate('agentTraces.customDataGrid.ariaLabel.closeFullScreen', {
            defaultMessage: 'Close full screen',
          })}
          onClick={onClose}
          display="empty"
          className="agentTracesCustomDataGrid__fullScreenCloseIcon"
        />
        <div className="agentTracesCustomDataGrid__fullScreenContent">{children}</div>
      </div>
    </EuiOverlayMask>
  );
};
...
export const RenderCustomDataGrid: React.FC<RenderCustomDataGridParams> = ({
  ...
  const toolbarControls = useMemo(() => {
    ...
    controls.push(
      ...
      >
        {isFullScreen
          ? i18n.translate('agentTraces.toolbarControls.exitFullScreen', {
              defaultMessage: 'Exit full screen',
            })
          : i18n.translate('agentTraces.toolbarControls.fullScreen', {
    ...
    <>
      <FullScreenWrapper isFullScreen={isFullScreen} onClose={() => setIsFullScreen(false)}>
        <div
          className={[
            isFullScreen
              ? 'agentTracesCustomDataGrid__fullWrapper'
              : 'agentTracesCustomDataGrid__normalWrapper',
          ]
            .filter(Boolean)
            .join(' ')}
          style={{
        ...
        >
          <div style={{ position: 'relative' }}>
            ...
            {secondaryToolbar.length > 0 && (
              <div className="agentTracesCustomDataGrid__secondaryToolbar">{secondaryToolbar}</div>
            )}
          </div>
          {isTableDataLoading && (
            <div className="agentTracesCustomDataGrid__gridLoadingOverlay">
