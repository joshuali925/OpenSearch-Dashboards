...
export const usePersistedChartState = (defaultState: DiscoverChartToggleId = 'histogram') => {
  ...
  useEffectOnce(() => {
    const storedValue = localStorage.getItem('agentTracesChartState');
    ...
    if (storedValue !== null) {
      ...
      if (!isValidState(storedValue)) {
        localStorage.setItem('agentTracesChartState', toggleIdSelected);
      }
    }
  });
  ...
  const updateToggleId = (newState: DiscoverChartToggleId) => {
    ...
    localStorage.setItem('agentTracesChartState', newState);
  };
