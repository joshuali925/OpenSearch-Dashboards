...
jest.mock('../../application/utils/hooks/use_current_agent_traces_id', () => ({
  useCurrentAgentTracesId: jest.fn(() => 'mockAgentTracesId'),
}));
...
describe('SaveAndAddButtonWithModal', () => {
  ...
  it('handles save and shows success toast', async () => {
    const mockedSaveSavedAgentTraces = saveSavedAgentTraces as jest.Mock;
    mockedSaveSavedAgentTraces.mockResolvedValue({ id: 'savedId123' });
    ...
    await waitFor(() => {
      expect(saveSavedAgentTraces).toHaveBeenCalled();
  ...
  it('handles save failure and shows error toast', async () => {
    const mockedSaveSavedAgentTraces = saveSavedAgentTraces as jest.Mock;
    mockedSaveSavedAgentTraces.mockRejectedValue(new Error('Save failed'));
