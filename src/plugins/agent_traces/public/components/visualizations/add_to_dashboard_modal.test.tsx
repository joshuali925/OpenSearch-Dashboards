...
jest.mock('../../application/utils/hooks/use_saved_agent_traces', () => ({
  useSavedAgentTraces: (id: string) => ({
    savedAgentTraces: { id, title: 'Old Title' },
  }),
}));
...
describe('AddToDashboardModal', () => {
    ...
    await act(async () => {
      render(
        <AddToDashboardModal
          ...
          savedAgentTracesId="agentTraces-1"
        />
      );
    });
  ...
  it('allows entering a title and selecting an existing dashboard', async () => {
    ...
    fireEvent.change(titleInput, { target: { value: 'My Saved Agent Traces' } });
    ...
    await waitFor(() =>
      expect(mockOnConfirm).toHaveBeenCalledWith(
        expect.objectContaining({
          newTitle: 'My Saved Agent Traces',
          mode: 'existing',
  ...
  it('allows entering a new dashboard name when "new" is selected', async () => {
    ...
    const titleInput = await screen.findByPlaceholderText('Enter save search name');
    fireEvent.change(titleInput, { target: { value: 'Agent Traces Title' } });
