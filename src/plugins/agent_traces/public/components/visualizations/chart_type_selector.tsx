    ...
    const options = allTypes.map((option) => ({
      ...
      inputDisplay: (
        <EuiFlexGroup gutterSize="s" alignItems="center">
          ...
          <EuiFlexItem>
            <EuiText
              data-test-subj={`agentTracesChartTypeSelector-${option.value}`}
              size="s"
              color={option.disabled ? 'subdued' : 'default'}
            >
              {option.inputDisplay}
  ...
  return (
    <EuiPanel hasBorder={false} hasShadow={false} paddingSize="s">
      <EuiFormRow
        ...
        label={i18n.translate('agentTraces.chartTypeSwitcher.title', {
          defaultMessage: 'Visualization type',
        })}
      >
        <EuiSuperSelect
          id="chartType"
          data-test-subj="agentTracesChartTypeSelector"
          compressed
