...
export const Range: React.FC<RangeProps> = ({ id, value, onChange, onDelete }) => {
  ...
  return (
    <EuiFlexGroup
      ...
      gutterSize="s"
      data-test-subj={`agentTracesVisThreshold-${id}`}
    >
      <EuiFlexItem>
        <EuiColorPicker
...
export const ThresholdCustomValues: React.FC<ThresholdCustomValuesProps> = ({
  ...
  return (
    <>
      <EuiSpacer size="s" />
      <EuiButton
        data-test-subj="agentTracesVisAddThreshold"
        onClick={handleAddRange}
        fullWidth
        size="s"
      >
        {i18n.translate('agentTraces.stylePanel.thresholdPanel.addThresholdButton', {
      ...
      <EuiFlexGroup
        ...
        justifyContent="center"
        gutterSize="s"
        data-test-subj="agentTracesVisThresholdBaseColor"
      >
        <EuiFlexItem>
          <EuiColorPicker
            swatches={THRESHOLD_COLORS}
