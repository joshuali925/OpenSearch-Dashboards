...
export class AgentTracesPlugin
  ...
  private readonly DISCOVER_VISUALIZATION_NAME = 'AgentTracesVisualization';
  ...
  public setup(
    ...
    this.registerAgentTracesVisualizationAlias(setupDeps);
    ...
    this.docViewsRegistry.addDocView({
      title: i18n.translate('agentTraces.docViews.trace.timeline.title', {
      ...
      shouldShow: (hit) => {
        ...
        return (
          currentPath.includes('/agentTraces/traces') || currentHash.includes('/agentTraces/traces')
        );
      },
    });
    ...
    this.docViewsLinksRegistry.addDocViewLink({
      label: i18n.translate(
        'agentTraces.discover.docTable.tableRow.viewSurroundingDocumentsLinkText',
        {
      ...
      generateCb: (renderProps: Record<string, unknown>) => {
        ...
        // Note: Agent Traces uses Redux for filter management, not filterManager
        // So we don't include filter state in URLs for context links
        const hash = stringify(
          url.encodeQuery({
            _g: rison.encode({}), // No global filters (agent traces uses Redux)
            _a: rison.encode({
              columns: (renderProps as any).columns,
              // No filters since agent traces uses Redux store instead of filterManager
            }),
          }),
    ...
    this.docViewsLinksRegistry.addDocViewLink({
      label: i18n.translate('agentTraces.discover.docTable.tableRow.viewSingleDocumentLinkText', {
  ...
  public start(core: CoreStart, plugins: AgentTracesStartDependencies): AgentTracesPluginStart {
    ...
    this.configureAgentTracesVisualizationVisibility(core, plugins).catch((error) => {
      // eslint-disable-next-line no-console
      console.error('Failed to configure agent traces visualization visibility', error);
    });
  ...
  private registerEmbeddable(
    ...
    const factory = new AgentTracesEmbeddableFactory(
      getStartServices,
      this.visualizationRegistryService
    );
  ...
  private registerAgentTracesVisualizationAlias(setupDeps: AgentTracesSetupDependencies) {
    const agentTracesVisDisplayName = i18n.translate('agentTraces.visualization.title', {
      defaultMessage: 'Visualize with Discover',
    });
    // Register agent traces visualization as visualization alias
    setupDeps.visualizations.registerAlias({
      name: this.DISCOVER_VISUALIZATION_NAME,
      aliasPath: '#/',
      aliasApp: PLUGIN_ID,
      title: agentTracesVisDisplayName,
      description: i18n.translate('agentTraces.visualization.description', {
        ...
        visualizations: {
          ...
          toListItem: ({ id, attributes, updated_at: updatedAt }) => {
            ...
            return {
              ...
              // TODO: it should navigate to different agent traces flavor based on the `attributes.type`
              editApp: `${PLUGIN_ID}/${AgentTracesFlavor.Traces}`,
  ...
  private async configureAgentTracesVisualizationVisibility(
    core: CoreStart,
    plugins: AgentTracesStartDependencies
  ) {
    const isAgentTracesEnabledWorkspace = await this.getIsAgentTracesEnabledWorkspace(core);
    if (isAgentTracesEnabledWorkspace) {
      ...
      // if current workspace has NO agentTraces.enabled, the agent traces visualization ingress should be hidden
