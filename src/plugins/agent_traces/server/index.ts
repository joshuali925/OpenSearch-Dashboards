...
export const config: PluginConfigDescriptor<ConfigSchema> = {
  ...
  deprecations: ({ rename, unused }) => [
    // This is to ensure that when agent traces is enabled, the necessary settings for the enhanced experience are also enabled;
    ...
    (settings, fromPath, addDeprecation) => {
      if (settings?.agentTraces?.enabled === true) {
