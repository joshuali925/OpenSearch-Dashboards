...
export const HierarchySpanCell = ({
  ...
  useEffect(() => {
    if (isRowSelected) {
      setCellProps?.({
        className: ['treeCell--firstColumn', 'agenticObsSpanDetailTable__selectedRow'],
      });
    } else {
  ...
  const ExpandCollapseIcon = () =>
    item?.children && item.children.length > 0 ? (
      <EuiIcon
        ...
        className="agenticObsSpanDetailTable__expandIcon"
        data-test-subj="treeViewExpandArrow"
      />
    ) : (
      <EuiIcon type="empty" className="agenticObsSpanDetailTable__hiddenIcon" />
    );
  ...
  const cellContent = (
    <div className="agenticObsSpanDetailTable__hierarchyCell" style={{ paddingLeft: indentation }}>
      <ExpandCollapseIcon />
      <SpanContent />
    </div>
  );
  ...
  return disableInteractions || !item ? (
    ...
    <button
      ...
      className="agenticObsSpanDetailTable__flyoutButton"
