...
export const SpanCell = ({
  ...
  useEffect(() => {
    if (
      ...
      setCellProps?.({ className: 'agenticObsSpanDetailTable__selectedRow' });
    } else {
      setCellProps?.({});
    }
    ...
    case 'status.code':
      return value === 2 ? (
        <EuiText color="danger" size="s">
          {i18n.translate('agenticObservability.spanDetailTable.errors.yes', {
            defaultMessage: 'Yes',
          })}
        </EuiText>
      ) : (
        i18n.translate('agenticObservability.spanDetailTable.errors.no', {
          defaultMessage: 'No',
        })
      );
    case 'spanId':
