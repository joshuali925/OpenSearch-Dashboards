...
describe('TimelineRuler', () => {
  ...
  it('should apply correct CSS classes for first, last, and center ticks', () => {
    ...
    expect(getByTestId('tick-label-0')).toHaveClass('agenticObsTimelineRuler__label--first');
    expect(getByTestId('tick-label-500')).toHaveClass('agenticObsTimelineRuler__label--center');
    expect(getByTestId('tick-label-1000')).toHaveClass('agenticObsTimelineRuler__label--last');
  });
  ...
  it('should render empty when no ticks are provided', () => {
    ...
    expect(container.querySelectorAll('.agenticObsTimelineRuler__tickContainer')).toHaveLength(0);
  });
});
