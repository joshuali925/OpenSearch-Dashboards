...
export const SpanIssuesTab: React.FC<SpanIssuesTabProps> = ({ selectedSpan }) => {
    ...
    return (
      <EuiPanel key={`issue-${index}`} paddingSize="s" color="subdued">
        ...
        {issue.details && (
          <>
            <EuiSpacer size="xs" />
            <div className="agenticObsSpanTabs__issuesContainer">
              <EuiCodeBlock
                language="json"
                paddingSize="s"
                isCopyable
                className="agenticObsSpanTabs__codeBlock"
              >
  ...
  if (!selectedSpan || isEmpty(selectedSpan)) {
    return (
      <EuiText color="subdued" textAlign="center">
        {i18n.translate('agenticObservability.spanIssuesTab.noSpanSelected', {
  ...
  if (issues.length === 0) {
    return (
      <EuiText color="subdued" textAlign="center">
        {i18n.translate('agenticObservability.spanIssuesTab.noIssues', {
