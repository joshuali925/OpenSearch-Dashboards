...
const FullScreenWrapper: React.FC<FullScreenWrapperProps> = ({
  ...
  return (
    <EuiOverlayMask>
      <div className="agenticObsCustomDataGrid__fullScreenWrapper">
        <EuiButtonIcon
          iconType="cross"
          aria-label={i18n.translate(
            'agenticObservability.customDataGrid.ariaLabel.closeFullScreen',
            {
          ...
          onClick={onClose}
          display="empty"
          className="agenticObsCustomDataGrid__fullScreenCloseIcon"
        />
        <div className="agenticObsCustomDataGrid__fullScreenContent">{children}</div>
      </div>
    </EuiOverlayMask>
  );
};
...
export const RenderCustomDataGrid: React.FC<RenderCustomDataGridParams> = ({
  ...
  const toolbarControls = useMemo(() => {
    ...
    controls.push(
      ...
      >
        {isFullScreen
          ? i18n.translate('agenticObservability.toolbarControls.exitFullScreen', {
              defaultMessage: 'Exit full screen',
            })
          : i18n.translate('agenticObservability.toolbarControls.fullScreen', {
    ...
    <>
      <FullScreenWrapper isFullScreen={isFullScreen} onClose={() => setIsFullScreen(false)}>
        <div
          className={[
            isFullScreen
              ? 'agenticObsCustomDataGrid__fullWrapper'
              : 'agenticObsCustomDataGrid__normalWrapper',
          ]
            .filter(Boolean)
            .join(' ')}
          style={{
        ...
        >
          <div style={{ position: 'relative' }}>
            ...
            {secondaryToolbar.length > 0 && (
              <div className="agenticObsCustomDataGrid__secondaryToolbar">{secondaryToolbar}</div>
            )}
          </div>
          {isTableDataLoading && (
            <div className="agenticObsCustomDataGrid__gridLoadingOverlay">
