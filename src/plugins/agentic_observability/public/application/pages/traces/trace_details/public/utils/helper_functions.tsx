...
export function getServiceInfo(selectedSpan: any, traceId?: string, isLoading?: boolean): string {
  if (isLoading) {
    return i18n.translate('agenticObservability.traceDetails.header.loading', {
  ...
  if (selectedSpan) {
    const serviceName =
      ...
      i18n.translate('agenticObservability.traceDetails.header.unknownService', {
        defaultMessage: 'Unknown Service',
      });
    const operationName =
      selectedSpan.name ||
      i18n.translate('agenticObservability.traceDetails.header.unknownOperation', {
        defaultMessage: 'Unknown Operation',
      });
    return `${serviceName}: ${operationName}`;
  } else if (traceId) {
    return i18n.translate('agenticObservability.traceDetails.header.unknownTrace', {
...
export function NoMatchMessage({ traceId }: NoMatchMessageProps) {
  return (
    <EuiCallOut
      title={i18n.translate('agenticObservability.traceView.callout.errorTitle', {
    ...
    >
      <p>
        {i18n.translate('agenticObservability.traceView.callout.errorDescription', {
          defaultMessage:
...
export function MissingFieldsEmptyState({
  ...
  return (
    <div style={{ padding: '24px' }}>
      <EuiCallOut
        title={i18n.translate('agenticObservability.traceDetails.missingFields.title', {
      ...
      >
        <EuiText size="s">
          <p>
            {i18n.translate('agenticObservability.traceDetails.missingFields.description', {
              defaultMessage:
            ...
            <EuiLink
              ...
              external
            >
              {i18n.translate('agenticObservability.traceDetails.missingFields.schemaLink', {
                defaultMessage: 'Data-Prepper OTel schema',
              })}
            </EuiLink>
            {i18n.translate(
              'agenticObservability.traceDetails.missingFields.descriptionContinued',
              {
                defaultMessage:
        ...
        <EuiText size="s">
          <p>
            <strong>
              {i18n.translate(
                'agenticObservability.traceDetails.missingFields.missingFieldsLabel',
                {
