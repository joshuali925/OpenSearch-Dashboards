 ...
 */
export const useSavedExplore = (agenticObsIdFromUrl?: string) => {
  ...
  const loadSavedExplore = useCallback(async () => {
    try {
      ...
      const savedExploreObject = await getSavedExploreById(agenticObsIdFromUrl);
  ...
  }, [agenticObsIdFromUrl, getSavedExploreById, toastNotifications]);
