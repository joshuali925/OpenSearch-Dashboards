...
export const usePersistedChartState = (defaultState: DiscoverChartToggleId = 'histogram') => {
  ...
  useEffectOnce(() => {
    const storedValue = localStorage.getItem('agenticObsChartState');
    ...
    if (storedValue !== null) {
      ...
      if (!isValidState(storedValue)) {
        localStorage.setItem('agenticObsChartState', toggleIdSelected);
      }
    }
  });
  ...
  const updateToggleId = (newState: DiscoverChartToggleId) => {
    ...
    localStorage.setItem('agenticObsChartState', newState);
  };
