...
describe('TableCell', () => {
  ...
  it('renders as time field cell when isTimeField is true', () => {
    ...
    expect(cell).toHaveClass('agenticObsDocTableCell', 'eui-textNoWrap');
  ...
  it('renders as regular field cell when isTimeField is false', () => {
    ...
    const cell = screen.getByTestId('docTableField');
    expect(cell).toHaveClass('agenticObsDocTableCell', 'eui-textTruncate');
    expect(cell).not.toHaveClass('eui-textNoWrap');
  });

  it('renders content wrapper for all fields', () => {
    render(<TableCell {...defaultProps} isTimeField={false} />);
    expect(document.querySelector('.agenticObsDocTableCell__content')).toBeInTheDocument();
  });
