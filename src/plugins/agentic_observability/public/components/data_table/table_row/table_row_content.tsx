...
const getCellClassName = (timeFieldName?: string, colName?: string): string => {
  const baseClass = 'agenticObsDocTableCell';
...
export const TableRowContent: React.FC<TableRowContentProps> = ({
  ...
  return (
    <tr
      key={row._id}
      className={row.isAnchor || isRowSelected ? 'agenticObsDocTable__row--highlight' : ''}
    >
      {isOnTracesPage ? (
        <td />
      ) : (
        <td
          ...
          className="agenticObsDocTableCell__toggleDetails"
        >
          <EuiButtonIcon
            color="text"
            ...
            aria-label={i18n.translate(
              'agenticObservability.defaultTable.docTableExpandToggleColumnLabel',
              {
