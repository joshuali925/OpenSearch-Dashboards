...
export function DiscoverFieldSearch({
  ...
  const searchPlaceholder = i18n.translate(
    'agenticObservability.discover.fieldChooser.searchPlaceHolder',
    {
      defaultMessage: 'Search field names',
    }
  );
  const aggregatableLabel = i18n.translate(
    'agenticObservability.discover.fieldChooser.filter.aggregatableLabel',
    {
      defaultMessage: 'Aggregatable',
    }
  );
  const searchableLabel = i18n.translate(
    'agenticObservability.discover.fieldChooser.filter.searchableLabel',
    {
      defaultMessage: 'Searchable',
    }
  );
  const typeLabel = i18n.translate('agenticObservability.discover.fieldChooser.filter.typeLabel', {
    defaultMessage: 'Type',
  });
  ...
  const filterBtnAriaLabel = isPopoverOpen
    ? i18n.translate(
        'agenticObservability.discover.fieldChooser.toggleFieldFilterButtonHideAriaLabel',
        {
          defaultMessage: 'Hide field filter settings',
        }
      )
    : i18n.translate(
        'agenticObservability.discover.fieldChooser.toggleFieldFilterButtonShowAriaLabel',
        {
    ...
    return (
      <EuiSelect
        ...
        aria-label={i18n.translate(
          'agenticObservability.discover.fieldChooser.filter.fieldSelectorLabel',
          {
  ...
  const compressedFieldSearch = (
    <EuiOutsideClickDetector onOutsideClick={() => {}} isDisabled={!isPopoverOpen}>
      <EuiCompressedFieldSearch
        ...
        value={value}
        className="agenticObsSideBar_searchInput"
      />
    </EuiOutsideClickDetector>
  );
  ...
  const fieldPopover = (
    <EuiPopover
      ...
      button={
        ...
        >
          {!isEnhancementsEnabledOverride && (
            <FormattedMessage
              id="agenticObservability.discover.fieldChooser.fieldFilterFacetButtonLabel"
    ...
    >
      <EuiPopoverTitle>
        {i18n.translate('agenticObservability.discover.fieldChooser.filter.filterByTypeLabel', {
          defaultMessage: 'Filter by type',
        })}
      </EuiPopoverTitle>
      {selectionPanel}
      <EuiPopoverFooter>
        <EuiCompressedSwitch
          label={i18n.translate(
            'agenticObservability.discover.fieldChooser.filter.hideMissingFieldsLabel',
            {
