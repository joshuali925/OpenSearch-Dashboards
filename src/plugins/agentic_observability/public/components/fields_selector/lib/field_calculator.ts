...
const getFieldValueCounts = (params: FieldValueCountsParams): FieldValueCounts => {
  ...
  if (NO_ANALYSIS_TYPES.includes(fieldType)) {
    return {
      error: i18n.translate(
        'agenticObservability.discover.fieldChooser.fieldCalculator.analysisIsNotAvailableForGeoFieldsErrorMessage',
        {
  ...
  try {
    ...
    if (hits.length === missing) {
      return {
        error: i18n.translate(
          'agenticObservability.discover.fieldChooser.fieldCalculator.fieldIsNotPresentInDocumentsErrorMessage',
          {
            defaultMessage:
  ...
  return values
    .filter((v) => {
      if (v instanceof Object && !Array.isArray(v)) {
        throw new Error(
          i18n.translate(
            'agenticObservability.discover.fieldChooser.fieldCalculator.analysisIsNotAvailableForObjectFieldsErrorMessage',
            {
