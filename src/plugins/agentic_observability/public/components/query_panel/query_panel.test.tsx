...
describe('QueryPanel', () => {
  ...
  it('shows loading progress when query is loading', () => {
    ...
    renderWithProvider(<QueryPanel />);
    expect(screen.getByTestId('agenticObsQueryPanelIsLoading')).toBeInTheDocument();
  });
  ...
  it('does not show loading progress when not loading', () => {
    ...
    renderWithProvider(<QueryPanel />);
    expect(screen.queryByTestId('agenticObsQueryPanelIsLoading')).not.toBeInTheDocument();
  });
