...
export const ActionButtons: React.FC<ActionButtonsProps> = ({
  ...
  const afterFeedbackTip = i18n.translate(
    'agenticObservability.resultsSummary.summary.afterFeedback',
    {
      defaultMessage:
    ...
    >
      {feedback !== FeedbackStatus.THUMB_DOWN && (
        <EuiSmallButtonIcon
          ...
          title={
            !feedback
              ? i18n.translate('agenticObservability.resultsSummary.summary.goodResponse', {
                  defaultMessage: `Good response`,
                })
              : afterFeedbackTip
          }
          onClick={() => onFeedback(true)}
          data-test-subj="agenticObsResultsSummary_summary_buttons_thumbup"
        />
      )}
      {feedback !== FeedbackStatus.THUMB_UP && (
        <EuiSmallButtonIcon
          ...
          title={
            !feedback
              ? i18n.translate('agenticObservability.resultsSummary.summary.badResponse', {
          ...
          data-test-subj="agenticObsResultsSummary_summary_buttons_thumbdown"
        />
      )}
      <div className="agenticObsResultsSummary__verticalSeparator" />
      <EuiCopy textToCopy={summary ?? ''}>
        {(copy) => (
          <EuiSmallButtonIcon
            aria-label="Copy to clipboard"
            title={i18n.translate('agenticObservability.resultsSummary.summary.copy', {
              defaultMessage: `Copy to clipboard`,
            })}
            onClick={copy}
            color="text"
            iconType="copy"
            data-test-subj="agenticObsResultsSummary_summary_buttons_copy"
          />
        )}
      </EuiCopy>
    </EuiFlexGroup>
