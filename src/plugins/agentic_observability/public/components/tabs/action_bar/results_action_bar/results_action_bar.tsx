...
export const DiscoverResultsActionBar = ({
  ...
  return (
    <EuiFlexGroup
      ...
      className="agenticObs-results-action-bar"
    ...
    >
            ...
            {dataset && rows?.length ? (
              ...
              >
                {isNonTableChart && dataset && rows?.length ? (
                  <EuiFlexItem grow={false}>
                    <EuiToolTip
                      content={i18n.translate('agenticObservability.discover.showRawDataTooltip', {
                        defaultMessage: 'View raw data table for this visualization',
                      })}
                    >
                      <EuiSwitch
                        label={i18n.translate('agenticObservability.discover.showRawData', {
                          defaultMessage: 'Show raw data',
                        })}
                        checked={!!showRawTable}
                        onChange={(e) => visualizationBuilder.setShowRawTable(e.target.checked)}
                        data-test-subj="agenticObsShowRawDataSwitch"
                      />
                    </EuiToolTip>
                  </EuiFlexItem>
                ) : null}
      ...
      <EuiFlexItem grow={false}>
        ...
        <EuiFlexGroup
          className="agenticObs-results-action-bar__extensions-container"
          direction="row"
          gutterSize="none"
          justifyContent="flexStart"
