...
describe('ThresholdCustomValues component', () => {
  ...
  it('renders base threshold', () => {
    ...
    expect(screen.getByTestId('agenticObsVisThresholdBaseColor')).toBeInTheDocument();
    expect(screen.getByTestId('agenticObsVisThreshold-0')).toBeInTheDocument();
  });
  ...
  it('updates threshold values', async () => {
    const { handleChange } = setup([{ value: 0, color: '#f8f8f8ff' }]);
    const valueInput = screen.getByTestId('agenticObsVisThresholdValue-0');
  ...
  it('deletes a threshold when trash icon is clicked', () => {
    const { handleChange } = setup([{ value: 0, color: '#f8f8f8ff' }]);
    const deleteButton = screen.getByTestId('agenticObsVisThresholdDeleteButton-0');
