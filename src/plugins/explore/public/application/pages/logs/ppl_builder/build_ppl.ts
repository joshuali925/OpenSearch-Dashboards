...
export function buildPPL(state: PPLBuilderState): string {
  ...
  if (state.aggregations.length > 0) {
    ...
    if (aggStr) {
      ...
      // Sort only follows an aggregation, and only while its column is still one
      // the query produces — so removing the metric a sort referenced silently
      // drops the now-dangling sort rather than emitting an invalid column.
      if (state.sort && sortableColumns(state).includes(state.sort.column)) {
        const sortClause = compileSort(state.sort);
