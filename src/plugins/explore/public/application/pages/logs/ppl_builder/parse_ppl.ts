...
export function parsePPL(query: string): PPLParseResult {
  ...
  try {
    ...
    // Beyond the source, the builder models a single `stats` command, optionally
    // followed by a single `sort` on one of the aggregated columns.
    ...
    for (const cmd of commandList) {
      ...
      if (sortCtx) {
        // A sort is modeled only after stats (it sorts the aggregated output),
