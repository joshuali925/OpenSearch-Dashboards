...
// The metrics tab must build the PromQL search request the same way the query
// tab does: preserve the live query state's dataset (and its data-connection
// `dataSource.meta`) and only swap in the executed query string. Without the
// meta the server-side PromQL search strategy can't route to the connection.
