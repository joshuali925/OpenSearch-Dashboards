...
/**
 * Extended parsed query with position information for editor integration.
 * Extends the base ParsedQuery from data/common with line/offset tracking.
 */
interface ParsedQueryWithPosition extends ParsedQuery {
 ...
 * This version extends the base splitMultiQueries with position tracking for editor integration.
 ...
 */
export function splitMultiQueries(queryString: string): ParsedQueryWithPosition[] {
  ...
  const queries: ParsedQueryWithPosition[] = [];
  let currentIndex = 0;
  let queryIndex = 0;
...
export function findQueryAtPosition(
  queryString: string,
  cursorOffset: number
): ParsedQueryWithPosition | undefined {
...
export function getQueryRelativePosition(
  queryString: string,
  cursorOffset: number
): { query: ParsedQueryWithPosition; relativeOffset: number } | undefined {
  const queries = splitMultiQueries(queryString);
