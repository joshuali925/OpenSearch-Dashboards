...
export class PrometheusClient {
  ...
  async getMetadata(): Promise<Record<string, MetricMetadata>> {
    const key = `metadata:${this.dataConnectionId}`;
    const cached = this.metadataCache.get(key);
    if (cached) return cached;
    const rc = this.getResourceClient();
    const raw = await rc.getMetricMetadata(this.dataConnectionId);
  ...
  async searchMetricNames(search: string): Promise<string[]> {
    const key = `search:${this.dataConnectionId}:${search}`;
    const cached = this.dataCache.get(key);
    if (cached) return cached;
    const escaped = search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const series = await this.getSeries(`{__name__=~".*${escaped}.*"}`);
    const names = Array.from(new Set(series.map((s) => s.__name__).filter(Boolean))).sort();
  ...
  async getLabelValues(label: string, metric?: string): Promise<string[]> {
    ...
    if (cached) return cached;
    const rc = this.getResourceClient();
    const data = await rc.getLabelValues(this.dataConnectionId, undefined, label);
    this.dataCache.set(key, data);
    return data;
  }
