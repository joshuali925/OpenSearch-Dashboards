...
export class ExpandDocumentTool implements MCPTool {
  ...
  public readonly description = 'Expand a document in the OpenSearch Dashboards explore logs view to see full details';
  ...
  public async execute(input: { documentId: string; action?: string }): Promise<any> {
    ...
    try {
      ...
      throw new Error(`Failed to ${action} document: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}