...
export class GetQueryStateTool implements MCPTool {
  ...
  public readonly description = 'Get the current query state from OpenSearch Dashboards explore interface';
  ...
  public async execute(input: { includeResults?: boolean }): Promise<any> {
    ...
    try {
      ...
      if (!globalServices || !globalServices.store) {
        this.logger.warn('GetQueryStateTool: Explore services not available, returning mock response');
        return {
          success: false,
      ...
      throw new Error(`Failed to get query state: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}