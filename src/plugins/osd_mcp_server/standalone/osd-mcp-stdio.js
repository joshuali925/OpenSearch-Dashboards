...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'The new query text to set (e.g., "SELECT * FROM logs WHERE level=\'ERROR\'")',
        },
        language: {
          type: 'string',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Optional query to run. If not provided, runs the current query in the interface',
        },
        waitForResults: {
          type: 'boolean',
    ...
    if (waitForResults) {
      ...
      return {
        ...
        message: query 
          ...
          : 'Current query execution started',
        executedQuery: currentQuery.query,
        language: currentQuery.language,
        dataset: currentQuery.dataset ? currentQuery.dataset.title : 'default',
        waitForResults,
...
async function handleExpandDocument(args) {
  ...
  return {
    ...
    rowIndex: rowIndex || 0,
    timestamp: new Date().toISOString(),
    note: 'This is a mock response. Real implementation would trigger document expansion in OpenSearch Dashboards UI.',
...
async function main() {
  ...
  console.error('💡 Note: Redux integration requires an active Explore page in OpenSearch Dashboards');
}
